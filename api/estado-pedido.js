...
module.exports = async (req,res)=>{
  ...
  try{
    ...
    const sql=getDb();
    const rows=await sql`SELECT order_number,product_title,quantity,total_amount,currency,status,payment_status,shipping_status,tracking_number,created_at,updated_at FROM orders WHERE order_number=${number} LIMIT 1`;
