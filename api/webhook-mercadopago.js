...
const { orderStatusFromPaymentEvent } = require('../lib/order-state');
  ...
  try {
    ...
    if (process.env.DATABASE_URL && payment.external_reference) {
      ...
      const rows = await sql`
        SELECT id,status,payment_status,customer_id,quantity,product_id
      ...
      if (rows.length) {
        ...
        const newStatus = orderStatusFromPaymentEvent(oldStatus, payment.status);
        await sql`
          UPDATE orders SET
        ...
        await sql`
          ...
          VALUES(
            ${order.id},
            ${body.action || 'payment.notification'},
            ${oldStatus},
            ${newStatus},
            ${JSON.stringify({
