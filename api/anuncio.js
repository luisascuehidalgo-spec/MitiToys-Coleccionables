...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const photo = (p,url,n,{ loading='lazy', fetchpriority='auto' }={}) => `<img src="${esc(url)}" alt="${esc(p.title)} · foto ${n+1}" width="800" height="800" loading="${loading}" fetchpriority="${fetchpriority}">`;
    const mosaic = products.slice(0,6).map((p,index) => `<figure>${photo(p,p.images[0],0,{ loading:index<3?'eager':'lazy', fetchpriority:index===0?'high':'auto' })}<figcaption>${esc(p.title)}</figcaption></figure>`).join('');
