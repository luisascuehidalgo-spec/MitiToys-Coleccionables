...
module.exports = async (req,res)=>{
  ...
  try{
    if(req.method==='GET'){
      const orders=await sql`SELECT o.id,o.order_number,o.product_id,COALESCE((SELECT STRING_AGG(oi.product_title||' x'||oi.quantity,' · ' ORDER BY oi.id) FROM order_items oi WHERE oi.order_id=o.id),o.product_title) AS product_title,o.quantity,o.unit_price,o.total_amount,o.currency,o.status,o.payment_id,o.payment_status,o.payment_status_detail,o.shipping_status,o.shipping_recipient,o.shipping_address,o.shipping_city,o.shipping_postal_code,o.shipping_phone,o.shipping_notes,o.shipping_carrier,o.tracking_number,o.created_at,o.updated_at,c.id AS customer_id,c.name AS customer_name,c.email AS customer_email,c.phone AS customer_phone,c.address AS customer_address,c.city AS customer_city,c.postal_code AS customer_postal_code FROM orders o LEFT JOIN customers c ON c.id=o.customer_id ORDER BY o.created_at DESC LIMIT 500`;
