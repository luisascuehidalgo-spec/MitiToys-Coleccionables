...
const AUTO_TERMINAL_NOTIFICATION_STATUSES = new Set(['sent', 'sending', 'delivery_uncertain', 'failed']);
...
async function sendNotification(sql, notification) {
  ...
  if (!emailEnabled()) {
    await sql`UPDATE notifications SET status='pending_configuration',updated_at=NOW() WHERE id=${notification.id} AND status NOT IN ('sent','sending','delivery_uncertain','failed')`;
