...
const AUTO_TERMINAL_NOTIFICATION_STATUSES = new Set(['sent', 'delivery_uncertain', 'failed']);
...
function terminalNotificationResult(notification) {
  ...
  if (status === 'delivery_uncertain') {
    return { sent: false, reason: 'delivery_uncertain', id: notification.provider_id || null };
  }
  if (status === 'failed') {
    return { sent: false, reason: 'failed', id: notification.provider_id || null };
  }
  return null;
}
...
async function sendNotification(sql, notification) {
  ...
  if (!emailEnabled()) {
    await sql`UPDATE notifications SET status='pending_configuration',updated_at=NOW() WHERE id=${notification.id} AND status NOT IN ('sent','delivery_uncertain','failed')`;
  ...
  if (!content) {
    return updateNotificationFailure(sql, notification, {
      status: 'failed',
