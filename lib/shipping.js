...
async function quoteEnviopack({ provinceCode, postalCode, packages }) {
  ...
  if (!validationResponse.ok) {
    console.error('Enviopack destination validation error:', 'status=' + validationResponse.status);
  ...
  if (!response.ok) {
    console.error('Enviopack quote error:', 'status=' + response.status);
...
async function createConfirmedShipment({ providerOrderId, order, packages, quote }) {
  ...
  if (!data?.id) throw Object.assign(new Error('No se pudo confirmar el identificador del envío creado en Envíopack.'), { code: 'SHIPPING_PROVIDER_UNCERTAIN' });
  return data;
}
