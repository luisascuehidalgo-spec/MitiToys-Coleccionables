...
function orderStatusFromShipping(currentStatus, paymentStatus, proposedStatus) {
  const current = String(currentStatus || 'pending');
  const payment = String(paymentStatus || 'pending');
  const proposed = String(proposedStatus || current);
...
function shippingStatusFromProvider(currentStatus, proposedStatus) {
  const current = String(currentStatus || 'not_shipped');
  const proposed = String(proposedStatus || current);

  if (current === 'delivered') return 'delivered';
  if (proposed === 'delivered') return 'delivered';
