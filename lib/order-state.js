...
function adminStatusError({ currentStatus, targetStatus, paymentId, paymentStatus, paymentStatusDetail }) {
  ...
  const payment = normalize(paymentStatus, 'pending');
  const paymentConflict = PAYMENT_CONFLICT_DETAILS.has(normalize(paymentStatusDetail));
  ...
  if (paymentConflict && PAID_OPERATIONAL_STATUSES.has(target)) {
    return 'El pedido tiene un conflicto entre pagos aprobados de Mercado Pago. Revisalo manualmente antes de continuar con fulfillment.';
  }
...
function adminStatusOptions(order) {
  ...
  if (hasPaymentConflict(order)) return [...options];
