...
function safeMercadoPagoCheckoutUrl(value) {
  try {
    const url = new URL(String(value || '').trim());
    const host = url.hostname.toLowerCase();
    const allowedHost = host === 'mercadopago.com'
    ...
    if (url.protocol !== 'https:' || !allowedHost || !standardPort || url.username || url.password) return '';
    return url.toString();
  } catch (_) {
    return '';
  }
...
async function findPreferenceByExternalReference(externalReference) {
  ...
  const initPoint = safeMercadoPagoCheckoutUrl(detail.init_point);
  if (!initPoint) return null;
  return { id: String(detail.id), init_point: initPoint };
}
...
async function createPreference(preference) {
  ...
  const data = await response.json().catch(() => ({}));
  const initPoint = safeMercadoPagoCheckoutUrl(data?.init_point);
  if (response.ok && data?.id && initPoint) {
    return { id: String(data.id), init_point: initPoint, recovered: false };
  }
