...
async function reserveStock(sql, { productId, orderId, quantity, reason = 'Reserva automática al iniciar una compra' }) {
  const qty = Math.floor(Number(quantity));
  if (!productId || !Number.isInteger(Number(orderId)) || Number(orderId) < 1 || !Number.isInteger(qty) || qty < 1) return [];
...
module.exports = { reserveStock, releaseReservedStock, releaseReservedStockIfUnshipped };
