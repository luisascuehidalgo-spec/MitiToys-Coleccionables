...
async function persistCheckoutLocal(sql, {
  ...
  const normalizedCustomerEmail = cleanValue(customerEmail, 160).toLowerCase();
  if (!Number.isInteger(Number(orderId)) || Number(orderId) < 1 || !normalizedCustomerEmail || !Array.isArray(items) || !items.length) {
      ...
      const queries = [
        ...
        txn`
          WITH customer_row AS (
            ...
            VALUES(
              ${cleanValue(customer?.name,120) || 'Cliente'},${normalizedCustomerEmail},${cleanValue(customer?.phone,50) || null},
              ${deliveryAddress || null},${deliveryCity || null},${provinceName},${finalPostalCode}
            )
          ...
          SELECT
            ...
            customer_row.id,${first.id},${summary},${units},${first.price},
